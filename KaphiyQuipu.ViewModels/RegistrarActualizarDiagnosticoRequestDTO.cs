using CoffeeConnect.Models;
using System;
using System.Collections.Generic;

namespace CoffeeConnect.DTO
{
    public class RegistrarActualizarDiagnosticoRequestDTO
    {
        #region Properties
        /// <summary>
        /// Gets or sets the DiagnosticoId value.
        /// </summary>
        public int DiagnosticoId
        { get; set; }

        /// <summary>
        /// Gets or sets the Numero value.
        /// </summary>
        public string Numero
        { get; set; }

        /// <summary>
        /// Gets or sets the SocioFincaId value.
        /// </summary>
        public int SocioFincaId
        { get; set; }

        /// <summary>
        /// Gets or sets the ObservacionInfraestructura value.
        /// </summary>
        public string ObservacionInfraestructura
        { get; set; }

        /// <summary>
        /// Gets or sets the ObservacionDatosCampo value.
        /// </summary>
        public string ObservacionDatosCampo
        { get; set; }

        /// <summary>
        /// Gets or sets the Responsable value.
        /// </summary>
        public string Responsable
        { get; set; }

        /// <summary>
        /// Gets or sets the TecnicoCampo value.
        /// </summary>
        public string TecnicoCampo
        { get; set; }

        /// <summary>
        /// Gets or sets the EmpresaId value.
        /// </summary>
        public int EmpresaId
        { get; set; }

        /// <summary>
        /// Gets or sets the AreaTotal value.
        /// </summary>
        public decimal? AreaTotal
        { get; set; }

        public string AreaCafeEnProduccion
        { get; set; }

        /// <summary>
        /// Gets or sets the Crecimiento value.
        /// </summary>
        public string Crecimiento
        { get; set; }

        /// <summary>
        /// Gets or sets the Bosque value.
        /// </summary>
        public string Bosque
        { get; set; }

        /// <summary>
        /// Gets or sets the Purma value.
        /// </summary>
        public string Purma
        { get; set; }

        /// <summary>
        /// Gets or sets the PanLlevar value.
        /// </summary>
        public string PanLlevar
        { get; set; }

        /// <summary>
        /// Gets or sets the Vivienda value.
        /// </summary>
        public string Vivienda
        { get; set; }

        /// <summary>
        /// Gets or sets the IngresoPromedioMensual value.
        /// </summary>
        public string IngresoPromedioMensual
        { get; set; }

        /// <summary>
        /// Gets or sets the IngresoAgricultura value.
        /// </summary>
        public string IngresoAgricultura
        { get; set; }

        /// <summary>
        /// Gets or sets the IngresoBodega value.
        /// </summary>
        public string IngresoBodega
        { get; set; }

        /// <summary>
        /// Gets or sets the IngresoTransporte value.
        /// </summary>
        public string IngresoTransporte
        { get; set; }

        /// <summary>
        /// Gets or sets the IngresoOtro value.
        /// </summary>
        public string IngresoOtro
        { get; set; }

        /// <summary>
        /// Gets or sets the PrestamoEntidades value.
        /// </summary>
        public string PrestamoEntidades
        { get; set; }

        /// <summary>
        /// Gets or sets the EstadoId value.
        /// </summary>
        public string EstadoId
        { get; set; }


        /// <summary>
        /// Gets or sets the UsuarioRegistro value.
        /// </summary>
        public string Usuario
        { get; set; }


        public List<DiagnosticoCostoProduccionTipo> DiagnosticoCostoProduccionList { get; set; }

        public List<DiagnosticoDatosCampoTipo> DiagnosticoDatosCampoList { get; set; }

        public List<DiagnosticoInfraestructuraTipo> DiagnosticoInfraestructuraList { get; set; }

        public RegistrarActualizarDiagnosticoRequestDTO()
        {
            DiagnosticoCostoProduccionList = new List<DiagnosticoCostoProduccionTipo>();
            DiagnosticoDatosCampoList = new List<DiagnosticoDatosCampoTipo>();
            DiagnosticoInfraestructuraList = new List<DiagnosticoInfraestructuraTipo>();
        }

        public string NombreArchivo { get; set; }
        public string PathArchivo { get; set; }

        #endregion
    }
}
