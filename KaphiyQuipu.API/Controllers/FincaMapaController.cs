...
namespace Integracion.Deuda.Controller
{
    ...
    [ApiController]
    public class FincaMapaController : ControllerBase
    {
        private IFincaMapaService _fincaMapaService;

        private Core.Common.Logger.ILog _log;
        public FincaMapaController(IFincaMapaService fincaMapaService, Core.Common.Logger.ILog log)
        {
            _fincaMapaService = fincaMapaService;
            _log = log;
        }
        ...
        public IActionResult Registrar(IFormFile file, [FromForm] string request)
        {
            ...
            RegistrarActualizarFincaMapaResponseDTO response = new RegistrarActualizarFincaMapaResponseDTO();
            try
            {

                var myJsonObject = JsonConvert.DeserializeObject<RegistrarActualizarFincaMapaRequestDTO>(request);
                response.Result.Data = _fincaMapaService.RegistrarFincaMapa(myJsonObject, file);
        ...
        public IActionResult Actualizar(IFormFile file, [FromForm] string request)
        {
            ...
            {
                var myJsonObject = JsonConvert.DeserializeObject<RegistrarActualizarFincaMapaRequestDTO>(request);
                response.Result.Data = _fincaMapaService.ActualizarFincaMapa(myJsonObject, file);
                response.Result.Success = true;
        ...
        [HttpPost]
        public IActionResult ConsultarPorFincaId([FromBody] ConsultaFincaMapaPorFincaIdRequestDTO request)
        {
            Guid guid = Guid.NewGuid();
            _log.RegistrarEvento($"{guid.ToString()}{Environment.NewLine}{Newtonsoft.Json.JsonConvert.SerializeObject(request)}");

            ConsultaFincaMapaPorIdResponseDTO response = new ConsultaFincaMapaPorIdResponseDTO();
            try
            {
                response.Result.Data = _fincaMapaService.ConsultarFincaMapaPorFincaId(request);
        ...
        public IActionResult DescargarArchivo([FromQuery(Name = "path")] string path, [FromQuery(Name = "name")] string name)
        {
            ...
            try
            {
                response.Result.Data = _fincaMapaService.DescargarArchivo(request);
                response.Result.Success = true;
        ...
        [HttpPost]
        public IActionResult ConsultarPorId([FromBody] ConsultaFincaMapaPorIdRequestDTO request)
        {
            Guid guid = Guid.NewGuid();
            _log.RegistrarEvento($"{guid.ToString()}{Environment.NewLine}{Newtonsoft.Json.JsonConvert.SerializeObject(request)}");
            ...
            try
            {
                response.Result.Data = _fincaMapaService.ConsultarFincaMapaPorId(request);
        ...
        [Route("Eliminar")]
        [HttpPost]
        public IActionResult Eliminar(RegistrarActualizarFincaMapaRequestDTO request)
        {
            Guid guid = Guid.NewGuid();
            ...
            try
            {
                ...
                response.Result.Data = _fincaMapaService.EliminarFincaMapa(request);
                response.Result.Success = true;
            }
            catch (ResultException ex)
            {
