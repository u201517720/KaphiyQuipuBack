...
namespace Integracion.Deuda.Controller
{
    ...
    [ApiController]
    public class NotaIngresoPlantaDocumentoAdjuntoController : ControllerBase
    {
        private INotaIngresoPlantaDocumentoAdjuntoService _NotaIngresoPlantaDocumentoAdjuntoService;

        private Core.Common.Logger.ILog _log;
        public NotaIngresoPlantaDocumentoAdjuntoController(INotaIngresoPlantaDocumentoAdjuntoService NotaIngresoPlantaDocumentoAdjuntoService, Core.Common.Logger.ILog log)
        {
            _NotaIngresoPlantaDocumentoAdjuntoService = NotaIngresoPlantaDocumentoAdjuntoService;
            _log = log;
        }
        ...
        {
            return Ok("Productor NotaIngresoPlanta Service. version: 1.20.01.03");
        }

        [Route("Registrar")]
        [HttpPost]
        //public IActionResult Registrar([FromBody] RegistrarActualizarSocioNotaIngresoPlantaCertificacionRequestDTO request, List<IFormFile> files)
        public IActionResult Registrar(IFormFile file, [FromForm] string request)
        {
            ...
            try
            {
                ...
                var myJsonObject = JsonConvert.DeserializeObject<RegistrarActualizarNotaIngresoPlantaDocumentoAdjuntoRequestDTO>(request);
                response.Result.Data = _NotaIngresoPlantaDocumentoAdjuntoService.RegistrarNotaIngresoPlantaDocumentoAdjunto(myJsonObject, file);
        ...
        public IActionResult Actualizar(IFormFile file, [FromForm] string request)
        {
            ...
            {
                var myJsonObject = JsonConvert.DeserializeObject<RegistrarActualizarNotaIngresoPlantaDocumentoAdjuntoRequestDTO>(request);
                response.Result.Data = _NotaIngresoPlantaDocumentoAdjuntoService.ActualizarNotaIngresoPlantaDocumentoAdjunto(myJsonObject, file);
                response.Result.Success = true;
        ...
        [Route("ConsultarPorNotaIngresoPlantaId")]
        [HttpPost]
        public IActionResult ConsultarPorNotaIngresoPlantaId([FromBody] ConsultaNotaIngresoPlantaDocumentoAdjuntoPorNotaIngresoPlantaIdRequestDTO request)
        {
            Guid guid = Guid.NewGuid();
            _log.RegistrarEvento($"{guid.ToString()}{Environment.NewLine}{Newtonsoft.Json.JsonConvert.SerializeObject(request)}");
            ...
            try
            {
                response.Result.Data = _NotaIngresoPlantaDocumentoAdjuntoService.ConsultarNotaIngresoPlantaDocumentoAdjuntoPorNotaIngresoPlantaId(request);
        ...
        public IActionResult DescargarArchivo([FromQuery(Name = "path")] string path, [FromQuery(Name = "name")] string name)
        {
            ...
            try
            {
                response.Result.Data = _NotaIngresoPlantaDocumentoAdjuntoService.DescargarArchivo(request);
                response.Result.Success = true;
