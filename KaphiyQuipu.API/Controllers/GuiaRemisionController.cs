...
namespace KaphiyQuipu.API.Controller
{
    ...
    public class GuiaRemisionController : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult ConsultarPorId([FromBody] ConsultarPorIdGuiaRemisionAcopioRequestDTO request)
        {
            Guid guid = Guid.NewGuid();
            _log.RegistrarEvento($"{guid}{Environment.NewLine}{JsonConvert.SerializeObject(request)}");
            ...
            try
            {
                response.Result.Data = _IGuiaRemisionAcopioService.ConsultarPorId(request);
                response.Result.Success = true;
            }
            catch (ResultException ex)
            {
