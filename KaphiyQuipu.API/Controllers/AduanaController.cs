...
namespace Integracion.Deuda.Controller
{
    ...
    [ApiController]
    public class AduanaController : ControllerBase
    {
        private IAduanaService _AduanaService;

        private Core.Common.Logger.ILog _log;
        public AduanaController(IAduanaService AduanaService, Core.Common.Logger.ILog log)
        {
        ...
        {
            return Ok("Aduana Service. version: 1.20.01.03");
        }

        //[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        [Route("Consultar")]
        [HttpPost]
        public IActionResult Consultar([FromBody] ConsultaAduanaRequestDTO request)
        {
            Guid guid = Guid.NewGuid();
            _log.RegistrarEvento($"{guid.ToString()}{Environment.NewLine}{Newtonsoft.Json.JsonConvert.SerializeObject(request)}");

            ConsultaAduanaResponseDTO response = new ConsultaAduanaResponseDTO();
            try
            {
                response.Result.Data = _AduanaService.ConsultarAduana(request);
